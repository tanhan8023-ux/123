...
要求：语气自然，简短有力，不要超过30个字。直接输出回复内容，不要有任何解释。`;
  ...
  // processAiResponseParts removed - now imported from chatUtils
          ...
          <div className="absolute inset-0 overflow-y-auto bg-white pb-[80px]">
            ...
            {(() => {
              const groupedPersonas = personas.reduce((acc, p) => {
                const groupName = p.group?.trim() || '星标朋友';
                if (!acc[groupName]) {
                  acc[groupName] = [];
                }
              ...
              const sortedGroups = Object.keys(groupedPersonas).sort((a, b) => {
                if (a === '星标朋友') return -1;
                if (b === '星标朋友') return 1;
              ...
              return (
                <>
                  {sortedGroups.length > 1 && (
                    <div className="px-3 py-2 flex gap-2 overflow-x-auto no-scrollbar border-b border-neutral-100 sticky top-0 bg-white z-40 shadow-sm">
                      <button
                  ...
                  {sortedGroups
                    .filter(g => selectedGroupFilter === null || selectedGroupFilter === g)
                    .map(groupName => (
                      <React.Fragment key={groupName}>
                        <div className="px-3 py-1 bg-neutral-100 text-[12px] text-neutral-500 font-medium sticky z-30" style={{ top: sortedGroups.length > 1 ? '41px' : '0' }}>
                          {groupName}
                        </div>
                        {groupedPersonas[groupName].map(p => (
                          <div 
                            key={p.id} 
                            onClick={() => {
                              setActiveTab('chat');
                            ...
                            className="flex items-center gap-3 p-3 border-b border-neutral-100 active:bg-neutral-50 cursor-pointer bg-white"
                          >
                            <div className="relative shrink-0">
